<template>
  <div class="graph-panel">
    <div class="panel-header">
      <span class="panel-title">{{ t('graph.panel.title') }}</span>
      
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" :title="t('graph.panel.refreshTitle')">
          <span class="icon-refresh" :class="{ 'spinning': loading }">↻</span>
          <span class="btn-text">{{ t('graph.panel.refresh') }}</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" :title="t('graph.panel.maximizeTitle')">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? t('graph.panel.liveMemory') : t('graph.panel.liveUpdating') }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ t('graph.panel.finishedProcessHint') }}</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" :title="t('graph.panel.dismissTitle')">
        ...
        <div v-if="selectedItem" class="detail-panel">
          <div class="detail-panel-header">
            <span class="detail-title">{{ selectedItem.type === 'node' ? t('graph.panel.nodeDetails') : t('graph.panel.relationship') }}</span>
          ...
          <div v-if="selectedItem.type === 'node'" class="detail-content">
            <div class="detail-row">
              <span class="detail-label">{{ t('graph.panel.name') }}</span>
              <span class="detail-value">{{ selectedItem.data.name }}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">{{ t('graph.panel.uuid') }}</span>
              <span class="detail-value uuid-text">{{ selectedItem.data.uuid }}</span>
            </div>
            <div class="detail-row" v-if="selectedItem.data.created_at">
              <span class="detail-label">{{ t('graph.panel.created') }}</span>
              <span class="detail-value">{{ formatDateTime(selectedItem.data.created_at) }}</span>
            </div>
            
            <!-- Properties -->
            <div class="detail-section" v-if="selectedItem.data.attributes && Object.keys(selectedItem.data.attributes).length > 0">
              <div class="section-title">{{ t('graph.panel.properties') }}</div>
              <div class="properties-list">
                <div v-for="(value, key) in selectedItem.data.attributes" :key="key" class="property-item">
                  <span class="property-key">{{ key }}:</span>
                  <span class="property-value">{{ value || t('graph.panel.noneValue') }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="detail-section" v-if="selectedItem.data.summary">
              <div class="section-title">{{ t('graph.panel.summary') }}</div>
              <div class="summary-text">{{ selectedItem.data.summary }}</div>
            </div>
            
            <!-- Labels -->
            <div class="detail-section" v-if="selectedItem.data.labels && selectedItem.data.labels.length > 0">
              <div class="section-title">{{ t('graph.panel.labels') }}</div>
          ...
          <div v-else class="detail-content">
            ...
            <template v-if="selectedItem.data.isSelfLoopGroup">
              <div class="edge-relation-header self-loop-header">
                {{ selectedItem.data.source_name }} - {{ t('graph.panel.selfRelations') }}
                <span class="self-loop-count">{{ selectedItem.data.selfLoopCount }} {{ t('graph.panel.items') }}</span>
              </div>
              ...
              <div class="self-loop-list">
                ...
                >
                  ...
                  >
                    ...
                    <span class="self-loop-name">{{ loop.name || loop.fact_type || t('graph.panel.relatedFallback') }}</span>
                  ...
                  <div class="self-loop-item-content" v-show="expandedSelfLoops.has(loop.uuid || idx)">
                    ...
                    <div class="detail-row" v-if="loop.fact">
                      <span class="detail-label">{{ t('graph.panel.fact') }}</span>
                      <span class="detail-value fact-text">{{ loop.fact }}</span>
                    </div>
                    <div class="detail-row" v-if="loop.fact_type">
                      <span class="detail-label">{{ t('graph.panel.type') }}</span>
                    ...
                    <div v-if="loop.episodes && loop.episodes.length > 0" class="self-loop-episodes">
                      <span class="detail-label">{{ t('graph.panel.episodes') }}</span>
            ...
            <template v-else>
              <div class="edge-relation-header">
                {{ selectedItem.data.source_name }} → {{ selectedItem.data.name || t('graph.panel.relatedTo') }} → {{ selectedItem.data.target_name }}
              </div>
              ...
              <div class="detail-row">
                <span class="detail-label">{{ t('graph.panel.label') }}</span>
                <span class="detail-value">{{ selectedItem.data.name || t('graph.panel.relatedTo') }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">{{ t('graph.panel.type') }}</span>
                <span class="detail-value">{{ selectedItem.data.fact_type || t('graph.panel.unknown') }}</span>
              </div>
              ...
              <div class="detail-section" v-if="selectedItem.data.episodes && selectedItem.data.episodes.length > 0">
                <div class="section-title">{{ t('graph.panel.episodes') }}</div>
              ...
              <div class="detail-row" v-if="selectedItem.data.valid_at">
                <span class="detail-label">{{ t('graph.panel.validFrom') }}</span>
      ...
      <div v-else-if="loading" class="graph-state">
        ...
        <p>{{ t('graph.panel.loadingGraph') }}</p>
      </div>
      
      
      <div v-else class="graph-state">
        <div class="empty-icon">❖</div>
        <p class="empty-text">{{ t('graph.panel.waitingOntology') }}</p>
      </div>
    </div>

    
    <div v-if="graphData && entityTypes.length" class="graph-legend">
      <span class="legend-title">{{ t('graph.panel.entityTypes') }}</span>
    ...
    <div v-if="graphData" class="edge-labels-toggle">
      ...
      <span class="toggle-label">{{ t('graph.panel.showEdgeLabels') }}</span>
    </div>
  </div>
</template>
