...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool, language: str = None) -> str:
        """System prompt for profile LLM (English or Chinese per self.language)."""
        lang = language or self.language
        if lang == 'en':
            return (
                "You are a social media user profile generation expert. Generate detailed, realistic "
                "personas for public opinion simulation, faithfully reproducing known real-world information. "
                "You must return valid JSON format, and all string values must not contain unescaped newline "
                "characters. Use English."
            )
        return (
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build group/organization persona user prompt (en/zh)."""
        lang = language or self.language
